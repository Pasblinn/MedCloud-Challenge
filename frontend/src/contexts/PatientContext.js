...
export const PatientProvider = ({ children }) => {
  ...
  const actions = {
    ...
    fetchPatients: useCallback(async (params = {}) => {
      try {
        ...
        const queryParams = {
          ...
          ...params // This will override the state values if provided
        };
    ...
    updateFilters: useCallback((newFilters) => {
      ...
      dispatch({ type: ActionTypes.SET_PAGINATION, payload: { page: 1 } });
    }, []),
  ...
  useEffect(() => {
    // Don't fetch on initial mount (handled above) or if already loading
    if (!state.loading) {
      actions.fetchPatients().catch(error => {
        console.warn('Failed to reload patients with filters:', error);
      });
    }
  }, [state.filters]); // Re-run when filters change
